...
router.post('/:token/accept', authMiddleware, (req, res) => {
  ...
  const { placeholder_id } = parsed.data;
  ...
  const tx = db.transaction(() => {
    if (placeholder_id) {
      ...
      db.prepare('UPDATE expense_splits SET user_id = ? WHERE user_id = ?').run(realUserId, placeholder_id);
      db.prepare('UPDATE expenses SET paid_by_user_id = ? WHERE paid_by_user_id = ?').run(realUserId, placeholder_id);
      db.prepare('UPDATE settlements SET payer_id = ? WHERE payer_id = ?').run(realUserId, placeholder_id);
      db.prepare('UPDATE settlements SET receiver_id = ? WHERE receiver_id = ?').run(realUserId, placeholder_id);
      db.prepare('DELETE FROM group_members WHERE group_id = ? AND user_id = ?').run(groupId, placeholder_id);
  ...
  return res.json({ slug: invite.group_slug, already_member: false });
});
