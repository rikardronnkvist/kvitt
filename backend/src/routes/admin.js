...
router.post('/users/:id/recovery-link', (req, res) => {
  const userId = Number(req.params.id);
  const user = db.prepare('SELECT id, full_name FROM users WHERE id = ? AND is_placeholder = 0').get(userId);
  if (!user) return res.status(404).json({ error: 'Användaren hittades inte.' });

  const passkeyCount = db.prepare('SELECT COUNT(*) AS count FROM passkeys WHERE user_id = ?').get(userId);
  if (Number(passkeyCount.count) === 0) {
    return res.status(400).json({ error: 'Användaren har inga passkeys. Använd registreringslänken istället.' });
  }

  const token = randomBytes(32).toString('hex');
  const expiresAt = new Date(Date.now() + 24 * 60 * 60 * 1000).toISOString();

  db.prepare('DELETE FROM user_recovery_tokens WHERE user_id = ? AND used_at IS NULL').run(userId);
  db.prepare('INSERT INTO user_recovery_tokens (user_id, token, expires_at) VALUES (?, ?, ?)').run(userId, token, expiresAt);

  tryLogActivity({
    eventType: 'admin.user.recovery_link_created',
    action: 'create',
    actorUserId: req.user.id,
    targetUserId: userId,
    entityType: 'user',
    entityId: userId,
    metadata: { target_user_full_name: user.full_name || null },
    ipAddress: resolveRequestIp(req),
  });

  return res.json({
    url: `${getFrontendPublicOrigin()}/recover?token=${token}`,
